...
extern unsigned char default_ip_tmpl[];
...
struct field {
  ...
  { "Hardware ID",    0x00, 0x10, NULL },
  { "Maker ID",       0x10, 0x10, NULL },
  { "Device Info",    0x20, 0x10, NULL },
  { "Area Symbols",   0x30, 0x08, check_areasym },
  { "Peripherals",    0x38, 0x08, NULL },
  { "Product No",     0x40, 0x0a, NULL },
  { "Version",        0x4a, 0x06, NULL },
...
void makeip(char *ip_tmpl, char *in, char *out)
{
  ...
  if (ip_tmpl == NULL)
  {
    memcpy(ip, default_ip_tmpl, sizeof(ip));
  }
  else 
  {
    fh = fopen(ip_tmpl, "rb");
...
int main(int argc, char *argv[])
{
  ...
  if(argc != 3 && argc != 4) 
  {
    fprintf(stderr, "Usage: %s ip.txt IP.BIN [IP.TMPL]\n", argv[0]);
    exit(1);
  }
